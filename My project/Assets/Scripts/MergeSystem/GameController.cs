...
public class GameController : MonoBehaviour
{
    ...
    void SendRayCast()  //마우스 클릭시 Ray 발사
    {
        ...
        if (Physics.Raycast(ray, out hit))                                      //hit 된 것이 있을 경우
        {
            ...
            {
                if (slot.itemObject.id == carryingItem.itemId)                  //슬롯 위에있는 아이템 id와 잡고 있는 아이템이 같을 경우
                {
    ...
    {
        var slot = GetSlotById(targetSlotId);           //기존 술롯에 있는 오브젝트를 가져와서 파괴
    ...
    void PlaceRandeomItem()                         //랜검한 슬롯에 아이템 배치
    {
        if (AllSlotsOccupied())
        {
    ...
    bool AllSlotsOccupied()                             //슬롯이 채워져 있는 확인 하는 함수
    {
        ...
        return true;                                    //다 차있으면 true를 리턴
    }
    Slot GetSlotById(int id)    //슬롯 ID로 슬롯을 검색
    {
        return slotDictionary[id];          //딕셔너리에 담겨있는 Slot Class 봔환(번호를 통해서0
    }
}
