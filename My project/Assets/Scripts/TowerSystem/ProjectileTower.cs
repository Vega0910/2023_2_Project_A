...
public class ProjectileTower : MonoBehaviour
{
    ...
    public float timeBetwwenShots = 1f;         //발사 간격
    private float shotCounter;                  //시간 설정

    private Transform target;                   //목표 타켓 설정
    public Transform launcherModel;             //모델 회전 시키기 위해서
    ...
    void Update()
    {
        ...
        if(shotCounter <= 0 && target != null )
        {
            ...
            firePoint.LookAt(target);                           //총알 발사는 Target을 바라본다.
        ...
        if (thisTower.enemiesUpdate)                //적을 배열 List에 검출했을때
        {
            if(thisTower.enemiesInRange.Count > 0)
            {
                ...
                foreach(EnemyConroller enemy in thisTower.enemiesInRange)
                {
                    if(enemy != null)
                    {
                        float distance = Vector3.Distance(transform.position, enemy.transform.position);
                        
                        if(distance < minDistance)  //가장 가까운 거리 갱신
                        {
